...
```sh
cd /mnt/alastria-node-besu/validator
...
<Configuration status="INFO" monitorInterval="5">
  ...
  <Appenders>
    ...
    <RollingFile name="RollingFile" fileName="/mnt/alastria-node-besu/besu/logs/besu.log" filePattern="/mnt/alastria-node-besu/besu/logs/besu-%d{yyyyMMdd}-%i.log.gz" >
        <PatternLayout pattern="%d{yyyy-MM-dd HH:mm:ss.SSSZZZ} | %t | %-5level | %c{1} | %msg %throwable{short.message}%n" />
        <!-- Logs in JSON format
...
[Service]
WorkingDirectory=/mnt/alastria-node-besu/validator/
Environment=LOG4J_CONFIGURATION_FILE=/mnt/alastria-node-besu/validator/config/besu/log-config.xml
Type=simple
User=besu
ExecStart=/mnt/alastria-node-besu/validator/bin/besu --config-file=/mnt/alastria-node-besu/validator/config/besu/config.toml
Restart=always
RestartSec=30
...
● besu.service - Service to init besu service on boot
 ...
 Main PID: 19089 (java)
   ...
   CGroup: /system.slice/besu.service
           └─19089 java -Dvertx.disableFileCPResolving=true -Dbesu.home=/mnt/alastria-node-besu/validator -Dlog4j.shutdownHookEnabled=false --add-
